...
cd $CUSTOM_BARRIO
for file in *tashrifat.*; do mv $file ${file//tashrifat/$CUSTOM_BARRIO}; done
for file in config/*/*tashrifat.*; do mv $file ${file//tashrifat/$CUSTOM_BARRIO}; done
mv {_,}$CUSTOM_BARRIO.theme
mv {_,}$CUSTOM_BARRIO.layouts.yml
grep -Rl tashrifat .|xargs sed -i '' -e "s/tashrifat/$CUSTOM_BARRIO/"
